...
spider_dir = r'/Users/chenzhihao/spider_data/grand_blue'
un_download_amt = 0


def parse_main_page(browser: WindowsChrome):
    page_url = r'https://www.maofly.com/manga/17393.html'
    html = browser.get(page_url, r'utf-8')
    soup = BeautifulSoup(html, 'html.parser')
    tab_content_div = soup.find('div', attrs={'id': 'comic-book-list'})
    # 只要单话的番外篇
    tad_pane_divs = tab_content_div.findAll('div', attrs={'class': 'tab-pane'})[2:3]
...
def test_parse_base_data(browser: WindowsChrome):
    url = r'https://www.maofly.com/manga/17393/141973.html'
    html = browser.get(url, 'utf-8')
    soup = BeautifulSoup(html, 'html.parser')
    script_tag_list = soup.findAll('script')
    for script_tag in script_tag_list:
...
if __name__ == '__main__':
    ...
    print(__get_file_name(2))
    print(__get_file_name(11))
    print(__get_file_name(111))
